# =============================================================================
# LESSON 1.3 | WORKSHOP | Exercise 4 of 4
# Threshold Tuning
# =============================================================================
# WHAT YOU WILL LEARN
# -------------------
# - How predict_proba() returns probability scores (not just 0/1 labels)
# - Why the default 0.5 threshold is not optimal for security use cases
# - How lowering the threshold increases recall but reduces precision
# - How to choose a threshold based on operational requirements
#
# RUN THIS FILE
# -------------
#   python module1_classic_ml/lesson3_logistic_regression/workshop/exercise4_threshold_tuning.py
# =============================================================================
...
# BACKGROUND
# =============================================================================
# model.predict() applies a hard threshold of 0.5: anything with P(phishing)
# >= 0.5 is flagged. But for security applications the cost of missing a
# phishing URL (False Negative) is typically much higher than the cost of a
# false alarm (False Positive). Lowering the threshold catches more phishing
# at the cost of more analyst reviews.
#
# model.predict_proba(X)[:, 1] returns the raw P(phishing) for each sample.
# We can then apply our own threshold to get labels.

# =============================================================================
# TASK 1 — Get Probability Scores
# =============================================================================
# Call model.predict_proba(X_test_scaled)[:, 1] to get P(phishing) for each URL.
# Build a DataFrame with columns: P(phishing), predicted_label (at 0.5), actual.
# Print the first 10 rows. Highlight any rows where prediction != actual.
...
# =============================================================================
# For thresholds [0.2, 0.3, 0.5, 0.7, 0.8], compute:
#   - precision (for phishing class)
#   - recall    (for phishing class)
#   - F1 score
#   - number of URLs flagged
...
# =============================================================================
# TASK 3 — Choose Threshold for 95% Recall
# =============================================================================
# Find the LOWEST threshold (sweeping from 0.0 to 0.9 in steps of 0.01) that
# still achieves recall >= 0.95.
# Print the threshold, the actual recall achieved, and the precision at that point.
...
# =============================================================================
# Use precision_recall_curve(y_test, probs) to get precision, recall, and
# thresholds. Plot the curve (recall on x-axis, precision on y-axis).
# Mark the default 0.5 threshold point with a red dot.
