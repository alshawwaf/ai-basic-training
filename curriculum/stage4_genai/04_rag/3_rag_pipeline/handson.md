...
This is the **same 6-document knowledge base** you used in Exercise 2. In 4.4.2 you got back the top-k chunks for each query; in this exercise you will take those exact chunks and hand them to the LLM from Lesson 4.3 as injected context. The `build_index` and `retrieve` functions are the same retrieval logic from Exercise 2, reproduced here so this file runs standalone. `QUESTIONS` are the three security questions you will answer using the RAG pipeline.
...
LLM provider: claude
Index built: ~22 chunks from 6 documents
```
(The exact chunk count depends on the chunking parameters — anywhere from 18 to 28 is normal.)
