# =============================================================================
# LESSON 2.1 | WORKSHOP | Exercise 4 of 4
# Scaling and Validation
# =============================================================================
# WHAT YOU WILL LEARN
# -------------------
# - How StandardScaler and MinMaxScaler differ
# - Why you MUST fit scalers on training data only
# - How to validate the final feature matrix before training
# - How to use sklearn Pipeline to prevent leakage automatically
#
# RUN THIS FILE
# -------------
#   python module2_intermediate/lesson1_feature_engineering/workshop/exercise4_scaling_and_validation.py
# =============================================================================
...
# --- Build the feature matrix from scratch (do not modify) ------------------
np.random.seed(42)
n = 200
...
# BACKGROUND
# =============================================================================
# StandardScaler: z = (x - mean) / std  → output is centred at 0, unit std.
# MinMaxScaler:   z = (x - min) / (max - min)  → output is in [0, 1].
# Both must be fit ONLY on training data. Fitting on test data or the full
# dataset leaks distribution information and produces overly optimistic results.

# =============================================================================
# TASK 1 — Assemble the Full Feature Matrix
# =============================================================================
# Combine: bytes_per_second, packet_rate, bytes_ratio, port_risk_score,
# proto_TCP, proto_UDP (from proto_dummies) into one DataFrame X.
# Print: shape, dtypes, missing values count, and .describe().
...
# =============================================================================
# TASK 2 — Scale with StandardScaler (Train Only)
# =============================================================================
# Split X and y into train/test (80/20, random_state=42).
# Fit a StandardScaler on X_train. Transform both.
# Verify: mean of each column in scaled training set ≈ 0.00, std ≈ 1.00.
...
# =============================================================================
# TASK 3 — StandardScaler vs MinMaxScaler Comparison
# =============================================================================
# Apply both scalers to X_train['bytes_per_second'] (as a 1-column array).
# Print summary stats for each. Intentionally add one extreme outlier to
# demonstrate how MinMaxScaler is more sensitive to it.
...
#   # Add an artificial extreme outlier
#   bps_with_outlier = bps.copy()
#   bps_with_outlier.iloc[0] = 500_000   # extreme value
#
...
# EXPECTED OUTPUT:
# StandardScaler:  mean=0.000, std=1.000, min~=-0.4, max=4.2  (outlier = 4.2σ)
# MinMaxScaler:    mean=0.006, std=0.020, min=0.000, max=1.000 (normal values all near 0!)

# =============================================================================
# TASK 4 (BONUS) — sklearn Pipeline
# =============================================================================
# Build a Pipeline: [('scaler', StandardScaler()), ('model', LogisticRegression())]
# Fit it on X_train / y_train. Score on X_test / y_test.
# Verify that you ONLY call pipeline.fit(X_train, y_train) — sklearn handles
# scaler fitting internally and never touches X_test during fit.
